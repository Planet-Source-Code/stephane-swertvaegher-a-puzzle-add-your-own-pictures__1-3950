VERSION 5.00
Begin VB.Form PForm3 
   BackColor       =   &H00E0E0E0&
   Caption         =   "Screen 2"
   ClientHeight    =   8310
   ClientLeft      =   165
   ClientTop       =   450
   ClientWidth     =   11880
   ControlBox      =   0   'False
   LinkTopic       =   "Form1"
   Moveable        =   0   'False
   ScaleHeight     =   554
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   792
   StartUpPosition =   2  'CenterScreen
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   119
      Left            =   4680
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   119
      Top             =   2970
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   118
      Left            =   5490
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   118
      Top             =   2970
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   117
      Left            =   6480
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   117
      Top             =   2925
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   116
      Left            =   7380
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   116
      Top             =   2925
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   115
      Left            =   4680
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   115
      Top             =   3735
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   114
      Left            =   5490
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   114
      Top             =   3735
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   113
      Left            =   6480
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   113
      Top             =   3690
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   112
      Left            =   7380
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   112
      Top             =   3690
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   111
      Left            =   4680
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   111
      Top             =   1485
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   110
      Left            =   5490
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   110
      Top             =   1485
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   109
      Left            =   6480
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   109
      Top             =   1440
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   108
      Left            =   7380
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   108
      Top             =   1440
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   107
      Left            =   4680
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   107
      Top             =   2250
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   106
      Left            =   5490
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   106
      Top             =   2250
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   105
      Left            =   6480
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   105
      Top             =   2205
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   104
      Left            =   7380
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   104
      Top             =   2205
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   103
      Left            =   4635
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   103
      Top             =   0
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   102
      Left            =   5445
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   102
      Top             =   0
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   101
      Left            =   6435
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   101
      Top             =   -45
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   100
      Left            =   7335
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   100
      Top             =   -45
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   99
      Left            =   4635
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   99
      Top             =   765
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   98
      Left            =   5445
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   98
      Top             =   765
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   97
      Left            =   6435
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   97
      Top             =   720
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   96
      Left            =   7335
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   96
      Top             =   720
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   95
      Left            =   4185
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   95
      Top             =   4770
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   94
      Left            =   4995
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   94
      Top             =   4770
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   93
      Left            =   5985
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   93
      Top             =   4725
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   92
      Left            =   6885
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   92
      Top             =   4725
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   91
      Left            =   4185
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   91
      Top             =   5535
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   90
      Left            =   4995
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   90
      Top             =   5535
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   89
      Left            =   5985
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   89
      Top             =   5490
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   88
      Left            =   6885
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   88
      Top             =   5490
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   87
      Left            =   4140
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   87
      Top             =   3285
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   86
      Left            =   4950
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   86
      Top             =   3285
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   85
      Left            =   5940
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   85
      Top             =   3240
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   84
      Left            =   6840
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   84
      Top             =   3240
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   83
      Left            =   4140
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   83
      Top             =   4050
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   82
      Left            =   4950
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   82
      Top             =   4050
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   81
      Left            =   5940
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   81
      Top             =   4005
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   80
      Left            =   6840
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   80
      Top             =   4005
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   79
      Left            =   4140
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   79
      Top             =   1800
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   78
      Left            =   4950
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   78
      Top             =   1800
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   77
      Left            =   5940
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   77
      Top             =   1755
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   76
      Left            =   6840
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   76
      Top             =   1755
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   75
      Left            =   4140
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   75
      Top             =   2565
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   74
      Left            =   4950
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   74
      Top             =   2565
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   73
      Left            =   5940
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   73
      Top             =   2520
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   72
      Left            =   6840
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   72
      Top             =   2520
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   71
      Left            =   4095
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   71
      Top             =   315
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   70
      Left            =   4905
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   70
      Top             =   315
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   69
      Left            =   5895
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   69
      Top             =   270
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   68
      Left            =   6795
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   68
      Top             =   270
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   67
      Left            =   4095
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   67
      Top             =   1080
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   66
      Left            =   4905
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   66
      Top             =   1080
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   65
      Left            =   5895
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   65
      Top             =   1035
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   64
      Left            =   6795
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   64
      Top             =   1035
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   63
      Left            =   4860
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   63
      Top             =   4770
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   62
      Left            =   5670
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   62
      Top             =   4770
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   61
      Left            =   6660
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   61
      Top             =   4725
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   60
      Left            =   7560
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   60
      Top             =   4725
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   59
      Left            =   4860
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   59
      Top             =   5535
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   58
      Left            =   5670
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   58
      Top             =   5535
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   57
      Left            =   6660
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   57
      Top             =   5490
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   56
      Left            =   7560
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   56
      Top             =   5490
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   55
      Left            =   4815
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   55
      Top             =   3285
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   54
      Left            =   5625
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   54
      Top             =   3285
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   53
      Left            =   6615
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   53
      Top             =   3240
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   52
      Left            =   7515
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   52
      Top             =   3240
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   51
      Left            =   4815
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   51
      Top             =   4050
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   50
      Left            =   5625
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   50
      Top             =   4050
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   49
      Left            =   6615
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   49
      Top             =   4005
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   48
      Left            =   7515
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   48
      Top             =   4005
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   47
      Left            =   4815
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   47
      Top             =   1800
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   46
      Left            =   5625
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   46
      Top             =   1800
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   45
      Left            =   6615
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   45
      Top             =   1755
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   44
      Left            =   7515
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   44
      Top             =   1755
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   43
      Left            =   4815
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   43
      Top             =   2565
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   42
      Left            =   5625
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   42
      Top             =   2565
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   41
      Left            =   6615
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   41
      Top             =   2520
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   40
      Left            =   7515
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   40
      Top             =   2520
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   39
      Left            =   4770
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   39
      Top             =   315
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   38
      Left            =   5580
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   38
      Top             =   315
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   37
      Left            =   6570
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   37
      Top             =   270
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   36
      Left            =   7470
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   36
      Top             =   270
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   35
      Left            =   4770
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   35
      Top             =   1080
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   34
      Left            =   5580
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   34
      Top             =   1080
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   33
      Left            =   6570
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   33
      Top             =   1035
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   32
      Left            =   7470
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   32
      Top             =   1035
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   31
      Left            =   4320
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   31
      Top             =   5085
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   30
      Left            =   5130
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   30
      Top             =   5085
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   29
      Left            =   6120
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   29
      Top             =   5040
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   28
      Left            =   7020
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   28
      Top             =   5040
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   27
      Left            =   4320
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   27
      Top             =   5850
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   26
      Left            =   5130
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   26
      Top             =   5850
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   25
      Left            =   6120
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   25
      Top             =   5805
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   24
      Left            =   7020
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   24
      Top             =   5805
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   23
      Left            =   4275
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   23
      Top             =   3600
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   22
      Left            =   5085
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   22
      Top             =   3600
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   21
      Left            =   6075
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   21
      Top             =   3555
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   20
      Left            =   6975
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   20
      Top             =   3555
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   19
      Left            =   4275
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   19
      Top             =   4365
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   18
      Left            =   5085
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   18
      Top             =   4365
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   17
      Left            =   6075
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   17
      Top             =   4320
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   16
      Left            =   6975
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   16
      Top             =   4320
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   15
      Left            =   4275
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   15
      Top             =   2115
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   14
      Left            =   5085
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   14
      Top             =   2115
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   13
      Left            =   6075
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   13
      Top             =   2070
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   12
      Left            =   6975
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   12
      Top             =   2070
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   11
      Left            =   4275
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   11
      Top             =   2880
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   10
      Left            =   5085
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   10
      Top             =   2880
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   9
      Left            =   6075
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   9
      Top             =   2835
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   8
      Left            =   6975
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   8
      Top             =   2835
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   7
      Left            =   4230
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   7
      Top             =   630
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   6
      Left            =   5040
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   6
      Top             =   630
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   5
      Left            =   6030
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   5
      Top             =   585
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   4
      Left            =   6930
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   4
      Top             =   585
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   3
      Left            =   4230
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   3
      Top             =   1395
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   2
      Left            =   5040
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   2
      Top             =   1395
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   1
      Left            =   6030
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   1
      Top             =   1350
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.PictureBox Pic1 
      AutoSize        =   -1  'True
      BackColor       =   &H8000000E&
      BorderStyle     =   0  'None
      Height          =   600
      Index           =   0
      Left            =   6930
      ScaleHeight     =   600
      ScaleWidth      =   690
      TabIndex        =   0
      Top             =   1350
      Visible         =   0   'False
      Width           =   690
   End
   Begin VB.Menu mnuoptions 
      Caption         =   "Options"
      Begin VB.Menu mnushowscr1 
         Caption         =   "Show Screen 1"
      End
   End
End
Attribute VB_Name = "PForm3"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit

Private Sub mnushowscr1_Click()
PForm3.Enabled = False
PForm3.Hide
frmMain.Enabled = True
frmMain.Show
End Sub
Private Sub pic1_MouseDown(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
If Button = 2 Then
Pic1(Index).Visible = False
frmMain.Pic1(Index).Visible = True
frmMain.Pic1(Index).Picture = Pic1(Index).Picture
frmMain.Pic1(Index).Left = Pic1(Index).Left
frmMain.Pic1(Index).Top = Pic1(Index).Top
End If
End Sub

Private Sub Pic1_MouseMove(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
If Button = 1 Then
Pic1(Index).Left = xpos - 4 - (Pic1(Index).Width / 2)
Pic1(Index).Top = ypos - 36 - (Pic1(Index).Height / 2)
End If
End Sub

